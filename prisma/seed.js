...
async function main() {
  ...
  // Create categories
  const categories = [
    { name: 'Adventure', description: 'Thrilling and exciting travel experiences' },
    ...
    { name: 'Food', description: 'Culinary journeys and gastronomic adventures' },
  ...
  // Create sample posts
  const adventureCategory = await prisma.category.findUnique({ where: { name: 'Adventure' } });
  ...
  const natureCategory = await prisma.category.findUnique({ where: { name: 'Nature' } });
  const cultureCategory = await prisma.category.findUnique({ where: { name: 'Culture' } });
      ...
      create: {
        ...
        categories: {
          connect: [
            ...
            natureCategory ? { id: natureCategory.id } : undefined
          ].filter(Boolean),
        },
      },
    });
    ...
    const post3 = await prisma.post.upsert({
      ...
      create: {
        ...
        categories: {
          connect: [
            { id: cityCategory.id }, 
            cultureCategory ? { id: cultureCategory.id } : undefined
          ].filter(Boolean),
        },
      },
    });
